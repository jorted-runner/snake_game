...
def threaded_client(conn, currentPlayer, food, portals):
    conn.send(pickle.dumps((snakes, food, portals)))
    while True:
        try:
...
while True:
    ...
    start_new_thread(threaded_client, (conn, currentPlayer, food, portals))
    currentPlayer += 1
